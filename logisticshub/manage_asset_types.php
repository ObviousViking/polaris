...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reactivate_type'])) {
        if (!$canDelete) {
            ...
            $stmt = $conn->prepare("UPDATE asset_types SET is_active = 1 WHERE id = ?");
            $stmt->bind_param("i", $reactivate_id);
            $stmt->execute();
            $stmt->close();
            log_audit_event($conn, 'asset_type', $reactivate_id, 'REACTIVATE', (int) $_SESSION['user_id']);
        ...
        if (!$canDelete) {
            ...
            $nameStmt = $conn->prepare("SELECT type_name FROM asset_types WHERE id = ?");
            ...
            // A type with any historical asset reference can't be
            // hard-deleted, so deactivate instead: drops out of the "add
            // asset" dropdown but past records still display correctly.
            $histStmt = $conn->prepare("SELECT COUNT(*) FROM assets WHERE asset_type_id = ?");
            ...
            if ($historicalCount > 0) {
                $stmt = $conn->prepare("UPDATE asset_types SET is_active = 0 WHERE id = ?");
                $stmt->bind_param("i", $delete_id);
                $stmt->execute();
                $stmt->close();
                log_audit_event($conn, 'asset_type', $delete_id, 'DEACTIVATE', (int) $_SESSION['user_id'], json_encode(['type_name' => $deletedName, 'reason' => 'has assets referencing this type']));
                ...
                $deleteStmt = $conn->prepare("DELETE FROM asset_types WHERE id = ?");
                $deleteStmt->bind_param("i", $delete_id);
                $deleteStmt->execute();
                $deleteStmt->close();
                log_audit_event($conn, 'asset_type', $delete_id, 'DELETE', (int) $_SESSION['user_id'], json_encode(['type_name' => $deletedName, 'reason' => $deleteReason]));
                    ...
                    <td>
                        ...
                        <form method="post" action="manage_asset_types.php<?php echo $embedded ? '?embedded=1' : ''; ?>"
                            ...
                            <button type="submit" class="action-btn delete-btn"
                                onclick="return confirmDeleteWithReason(this.form, 'Delete this type? If it has any asset history it will be deactivated instead of deleted.')">Delete</button>
                        </form>
                        <?php elseif ($canDelete): ?>
                        <form method="post" action="manage_asset_types.php<?php echo $embedded ? '?embedded=1' : ''; ?>"
                            style="display:inline;">
                            <input type="hidden" name="reactivate_type" value="<?php echo $type['id']; ?>">
