...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($type_name === '') {
        ...
        $check = $conn->prepare("SELECT COUNT(*) FROM exhibit_types WHERE UPPER(type_name) = ? AND exhibit_type_id != ?");
        ...
        if ($count > 0) {
            ...
            if ($type_id > 0) {
                $stmt = $conn->prepare("UPDATE exhibit_types SET type_name = ? WHERE exhibit_type_id = ?");
                $stmt->bind_param("si", $type_name, $type_id);
                $stmt->execute();
                $stmt->close();
                log_audit_event($conn, 'exhibit_type', $type_id, 'UPDATE', (int) $_SESSION['user_id'], json_encode(['type_name' => $type_name]));
                $message = "Exhibit type updated.";
            } else {
                $stmt = $conn->prepare("INSERT INTO exhibit_types (type_name) VALUES (?)");
                $stmt->bind_param("s", $type_name);
                $stmt->execute();
                $newId = $conn->insert_id;
                $stmt->close();
                log_audit_event($conn, 'exhibit_type', $newId, 'CREATE', (int) $_SESSION['user_id'], json_encode(['type_name' => $type_name]));
                $message = "New exhibit type added.";
            }
        }
    }
}
...
$types = [];
$res = $conn->query("SELECT * FROM exhibit_types ORDER BY type_name");
...
<style>
    ...
    <div class="container">
        <h2>Manage Exhibit Types</h2>

        <?php if (!empty($message)): ?>
        <div class="message"><?php echo htmlspecialchars($message); ?></div>
        <?php endif; ?>

        <form method="post" action="manage_exhibit_types.php<?php echo $embedded ? '?embedded=1' : ''; ?>">
                ...
                <tr>
                    ...
                    <td>
                        <button class="action-btn"
                            onclick="populateForm('<?php echo $type['exhibit_type_id']; ?>', '<?php echo htmlspecialchars($type['type_name'], ENT_QUOTES); ?>')">Edit</button>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
